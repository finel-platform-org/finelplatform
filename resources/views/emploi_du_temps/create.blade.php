<x-app-layout>
    ...
    $.get('/get-semestres-by-niveau/' + id, function (data) {
        ...
        // NEW: Semester change handler for modules
    $('#semestre').on('change', function() {
        let semesterId = $(this).val();
        
        if (!semesterId) return;
        
        $.get('/get-modules-by-semester/' + semesterId, function(data) {
        ...
        $('#section').on('change', function () {
            ...
            // Load modules
            $.get('/get-modules-by-section/' + id, function (data) {
                $('#module').empty().append('<option disabled selected>Choisir un module</option>');
                data.forEach(m => $('#module').append(`<option value="${m.ModuleID}">${m.Nom}</option>`));
            });
        });

        // Activite change handler
